...
export async function POST(request: Request) {
  ...
  const body = await request.json(); const slots = Array.isArray(body.slots) ? body.slots : [];
  if (!body.name || !body.email || !body.phone || !slots.length || slots.some((slot: { key?: unknown }) => typeof slot.key !== "string" || !/^2026-08-(29|30)T\d{2}:\d{2}$/.test(slot.key))) return NextResponse.json({ error: "Please complete your contact details and choose an available time." }, { status: 400 });
