...
export function BookingSlotPicker() {
  ...
  return <><section className="availability"><p className="eyebrow">Weekend availability</p><h2>Choose every block<br /><em>you need.</em></h2><p className="availability-note">Every button is a 30-minute block. Pick two for one hour, or select more blocks for a longer session or multiple dancers.</p><div className="day-grid">{dayCard("Saturday, August 29", saturdayTimes)}{dayCard("Sunday, August 30", sundayTimes)}</div></section><section className="booking-register" id="register"><div><p className="eyebrow">Your booking</p><h2>Build the session<br /><em>that works.</em></h2><p>Select the in-person coaching type, then every time block you need. Stripe checkout will be added next to secure your appointment.</p><label className="booking-session-label">Coaching type<select value={sessionType} onChange={(event) => setSessionType(event.target.value)}><option value="in-person-one">In person · one dancer</option><option value="in-person-two">In person · two dancers</option></select></label></div><div className="booking-summary"><p>Your selected time</p>{selectedSlots.length ? <><strong>{selectedSlots.length} block{selectedSlots.length === 1 ? "" : "s"} · {duration}</strong><ul>{selectedSlots.map((slot) => <li key={slot}>{slot}</li>)}</ul></> : <span>Choose one or more time blocks above.</span>}<button type="button" className="button" disabled={!selectedSlots.length}>Continue to booking <span>→</span></button><small>Your selected time will be locked after secure payment is connected.</small></div></section></>;
}
